...
* ``YT_server_200Hz.h5``（约 15.1 GB）：``train``/``valid``/``test`` 三个组，分别含 1479/422/213 条切片；
* ``SimpleDemo.h5``（约 0.55 GB）：同格式的小样本，32/9/6 条切片；
* 每条切片是一个组，含 ``acc``（m/s²）、``gyr``（rad/s）、``ts``（Unix 秒）、``pos``（m，真值）、
  ``qua``（真值四元数，device→world，wxyz）、``acc_bias``/``gyr_bias``/``mag``/``motion_qua``（发布数据中全为 0）；
* ``para/mean``、``para/std``：导航系下加速度/陀螺的全数据集均值与标准差（归一化用）。

命名规律：YT 为 ``<受试者缩写>_<楼层>_server_<录制号>_<切片号>``，Demo 为 ``mid360_<日期>_<时间>_<切片号>``。

已核实的约定（Zenodo 说明 + 物理自检）：机体系比力含重力；``qua`` 为 ``body_to_world`` wxyz；
世界系 z 轴向上；200 Hz；真值来源未在公开资料中注明（Demo 文件名 ``mid360`` 暗示 Livox Mid-360 激光方案）。
...
DEMO_NAME = re.compile(r"^(?P<rig>mid360)_(?P<date>\d{4}-\d{2}-\d{2})_(?P<time>\d{6})_(?P<slice>\d+)$")
GROUPED_SEED = 0
...
def _root(source) -> Path:
    ...
    for cand in (source, source / "PedLocData"):
        if any((cand / name).exists() for name in FILES.values()):
            return cand
    raise FileNotFoundError(f"PedLocData files ({', '.join(FILES.values())}) not found under {source}")
...
def parse_name(prefix: str, name: str) -> dict:
    ...
    if prefix == "yt":
        ...
        if m:
            return {"subject": m["subject"], "floor": m["floor"], "recording": f"{m['subject']}_{m['floor']}_server_{m['rec']}",
                    "slice": int(m["slice"]), "group": f"yt_{m['subject']}"}
    else:
        m = DEMO_NAME.match(name)
        if m:
            rec = f"mid360_{m['date']}_{m['time']}"
            return {"subject": "unknown", "floor": "unknown", "recording": rec, "slice": int(m["slice"]),
                    "group": f"demo_{m['date']}_{m['time']}"}
    return {"subject": "unknown", "floor": "unknown", "recording": name, "slice": -1, "group": f"{prefix}_{name}"}
...
def list_sequences(source) -> list:
    """全部可转换的 ``sequence_id``（``yt_*`` / ``demo_*``）；两个文件的切片都位于 train/valid/test 组中。"""
...
def official_splits(source) -> dict:
    """官方划分（``valid`` → ``val``），另附 ``test_yt`` / ``test_demo`` 子集。**存在会话级泄漏**，见模块文档。"""
        ...
        with h5py.File(path, "r") as handle:
            for group, split in SPLIT_GROUPS.items():
                ...
                for name, node in handle[group].items():
                    ts = node["ts"]
                    pos = node["pos"]
                    row = {"sequence_id": f"{prefix}_{name}", "prefix": prefix, "split": split, "n": int(ts.shape[0]),
                           "t0": float(ts[0]), "t1": float(ts[-1]), "p0": pos[0].tolist(), "p1": pos[-1].tolist()}
...
def audit_official_splits(source, rows: Optional[list] = None) -> dict:
    ...
    for prefix in sorted({r["prefix"] for r in rows}):
        ...
        for split in ("train", "val", "test"):
            ...
            adjacent = [r for r in items if any(x["split"] != split and abs(x["slice"] - r["slice"]) == 1
        ...
        report[prefix] = {
            ...
            "recordings_spanning_splits": sum(len({x["split"] for x in v}) > 1 for v in by_rec.values()),
            "consecutive_slice_pairs": len(gaps),
            "consecutive_pairs_in_different_splits": cross_pairs,
            "gap_s_percentiles_0_50_100": [round(float(x), 3) for x in np.percentile(gaps, [0, 50, 100])] if gaps else [],
            "jump_m_percentiles_0_50_100": [round(float(x), 3) for x in np.percentile(jumps, [0, 50, 100])] if jumps else [],
...
def grouped_splits(source, rows: Optional[list] = None, seed: int = GROUPED_SEED,
    ...
    每个文件内部分别处理：固定种子打乱分组后，按时长依次填满 ``test``（约 10%）与 ``val``（约 20%），
    其余为 ``train``。返回的键与官方划分相同，另含 ``test_yt`` / ``test_demo``。
    """
...
def read_slice(node, prefix: str, name: str, split: str, source_file: str, group: str = "unknown") -> RawSequence:
    ...
    attrs = {
        ...
        "placement": "unknown",
        "group_id": info["group"],
        "position_source": "undocumented reference trajectory (PedLocData 'Ground truth position'; demo files are "
                           "named mid360, suggesting Livox Mid-360 LiDAR-based SLAM)",
        "orientation_source": "undocumented reference (PedLocData 'Ground truth quaternion device to world')",
        ...
        "floor": info["floor"],
        "imu_calibration": "unknown (acc_bias/gyr_bias fields are all zero in the release; no calibration applied)",
    }
    ...
    if len(lengths) != 1:
        return rig.rejected_sequence(sequence_id, f"inconsistent array lengths {sorted(lengths)}", attrs, notes)
    for key in ("acc_bias", "gyr_bias", "mag", "motion_qua"):
        if key in node:
            values = node[key][()]
            if np.any(values != 0):
                notes.append(f"warning: '{key}' contains non-zero values (expected all-zero placeholder); ignored")
    notes.append("acc_bias/gyr_bias/mag/motion_qua are all-zero placeholders in the release and are ignored")
    ...
    if pose_valid.any() and np.abs(norms[pose_valid] - 1).max() > 1e-3:
        notes.append(f"quaternions renormalised (max norm deviation {np.abs(norms[pose_valid] - 1).max():.2e})")
            ...
            for group, split in SPLIT_GROUPS.items():
                ...
                for name in sorted(handle[group].keys()):
                    ...
                    try:
                        ...
                        yield rig.rejected_sequence(sequence_id, f"parse error: {exc!r}",
                                                    {"source_license": LICENSE, "source_files": f"{filename}:/{group}/{name}"})
                        continue
