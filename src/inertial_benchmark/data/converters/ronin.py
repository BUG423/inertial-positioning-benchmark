...
* ``synced/*`` 是手机（IMU 设备）数据，200 Hz，``synced/time`` 为手机系统时间（秒）；
  ``pose/*`` 与 ``synced/time`` 逐行对应（README："timestamp for data is synced/time"）；
* 陀螺：``synced/gyro_uncalib − imu_init_gyro_bias``；
  加速度：``imu_acce_scale ⊙ (synced/acce − imu_acce_bias)``；
* ``pose/tango_pos`` 是**胸前 Tango 设备**（另一个刚体）的 VIO 位置，
  世界系为 Tango 起始系（重力对齐，z 向上）；
  ``pose/tango_ori`` 是 Tango 设备姿态，**不是手机姿态**，不能作为 IMU 机体系的参考姿态；
* 手机在 Tango 世界系中的姿态由官方公式得到：
  ``q_wb(t) = tango_ori[0] ⊗ start_calibration ⊗ src[0]* ⊗ src(t)``，
  其中 ``src`` 为所选姿态源（game_rv / EKF / 陀螺积分），
  ``start_calibration`` 是起始校准段内手机→Tango 的旋转；
* 姿态源选择与官方训练模式一致
  （``select_orientation_source``，``max_ori_error=20``，``grv_only=False``）：
  game_rv 末端对齐误差 < 20° 时用 game_rv，
  否则取 ``[gyro_integration, game_rv, ekf]`` 中误差最小者；
  官方测试模式只用 game_rv，对应 IPB 的 ``imu/orientation``
  （原始 game_rv，与参考世界系差常值偏航）；
...
_OFFICIAL_LISTS = {
    "train": """
        a001_1 a001_3 a002_1 a002_2 a003_1 a003_2 a004_2 a005_1 a007_2 a007_3 a009_3 a010_1 a010_3
        a011_1
        a011_3 a012_1 a012_3 a013_1 a013_3 a014_1 a015_1 a015_3 a016_3 a017_1 a017_3 a018_1 a018_2
        a018_3
        a020_1 a020_3 a021_1 a022_1 a022_3 a023_2 a023_3 a025_1 a025_3 a026_3 a027_1 a027_3 a028_3
        a030_1
        a031_1 a031_3 a033_1 a033_3 a034_1 a034_2 a034_3 a035_1 a036_1 a036_3 a037_3 a038_3 a040_2
        a040_3
        a043_1 a043_3 a044_1 a044_3 a045_1 a045_3 a046_1 a046_3 a047_3 a000_1 a000_10 a000_2 a000_3
        a000_4
        a000_5 a059_1 a059_3
    """,
    "val": """
        a000_6 a000_8 a000_9 a009_2 a014_3 a015_2 a021_3 a026_1 a028_1 a038_1 a039_1 a039_2 a045_2
        a047_1
        a056_1 a056_3
    """,
    "test_seen": """
        a001_2 a003_3 a004_3 a005_3 a009_1 a010_2 a011_2 a012_2 a013_2 a014_2 a016_1 a017_2 a020_2
        a021_2
        a022_2 a023_1 a025_2 a026_2 a027_2 a030_3 a031_2 a033_2 a035_3 a036_2 a037_1 a038_2 a044_2
        a046_2
        a047_2 a000_7 a000_11 a059_2
    """,
    "test_unseen": """
        a006_2 a019_3 a024_1 a024_3 a029_1 a029_2 a032_1 a032_3 a042_2 a049_1 a049_2 a049_3 a050_1
        a050_3
        a051_1 a051_2 a051_3 a052_2 a053_1 a053_2 a053_3 a054_1 a054_2 a054_3 a055_2 a055_3 a057_1
        a057_2
...
def _discover(source: Path) -> Dict[str, Path]:
    """查找含 ``info.json`` 与 ``data.hdf5`` 的序列目录（``source/<seq>`` 或
    ``source/<子集>/<seq>``）。
    """
    ...
    found: Dict[str, Path] = {}
    candidates = (
        [source] + sorted(p for p in source.iterdir() if p.is_dir()) if source.is_dir() else []
    )
        ...
        for seq in sorted(p for p in folder.iterdir() if p.is_dir()):
            if (seq / "info.json").is_file() and (seq / "data.hdf5").is_file():
                if seq.name in found and found[seq.name] != seq:
                    raise ValueError(
                        f"duplicate RoNIN sequence {seq.name}: {found[seq.name]} and {seq}"
                    )
                found[seq.name] = seq
    return found
...
def list_sequences(source: Path) -> List[str]:
    """契约可选成员：本地可转换的全部 sequence_id（排序）。

    包括不在官方划分中的序列；本地 152 条均在官方列表中。
    """
...
def official_splits(source: Path) -> Dict[str, List[str]]:
    ...
    return {
        key: [name for name in names if name in available]
        for key, names in _official_lists().items()
    }
...
def load_sequence(folder: Path, root: Optional[Path] = None) -> RawSequence:
    ...
    with h5py.File(folder / "data.hdf5", "r") as handle:
        ...
        ekf = (
            np.asarray(handle["pose/ekf_ori"], dtype=np.float64)
            if "pose/ekf_ori" in handle
            else None
        )
    n = len(time)
    for label, arr in [
    ...
    if source_name == "gyro_integration":
        # 官方从第 0 帧、以 game_rv[0] 为初值积分去零偏陀螺；
        # 此处用零阶保持指数映射积分（与官方欧拉积分差异可忽略）
    ...
    init_rotor = pu.quat_mul(
        pu.quat_mul(pu.quat_normalize(tango_ori[0]), start_calib), pu.quat_conj(src[0])
    )
    reference = pu.quat_mul(init_rotor, src)
    grv_rotor = pu.quat_mul(
        pu.quat_mul(pu.quat_normalize(tango_ori[0]), start_calib), pu.quat_conj(game_rv[0])
    )
    ...
    notes = [
        f"gyroscope = synced/gyro_uncalib - imu_init_gyro_bias {np.round(gyro_bias, 6).tolist()} "
        "(official)",
        f"accelerometer = imu_acce_scale {np.round(acce_scale, 6).tolist()} * (synced/acce - "
        "imu_acce_bias "
        f"{np.round(acce_bias, 6).tolist()}) (official)",
        f"dropped {start} pre-calibration frames before start_frame (official)",
        "pose/orientation is the PHONE attitude, not pose/tango_ori (tango_ori belongs to the "
        "body-mounted Tango device)",
        f"reference orientation source = {source_name} (official selection rule, "
        f"max_ori_error={MAX_GRV_ERROR_DEG:g} deg; "
        f"end-alignment errors: game_rv={info['grv_ori_error']:.2f}, "
        f"ekf={info['ekf_ori_error']:.2f}, "
        f"gyro_integration={info['gyro_integration_error']:.2f} deg)",
        "reference orientation aligned to Tango world at frame 0: tango_ori[0] * "
        "start_calibration * src[0]^-1 * src(t)",
    ...
    if source_name == "gyro_integration":
        notes.append(
            "WARNING: gyro_integration has no gravity correction, so its tilt drifts; the physics "
            "check decides "
            "whether this reference is usable (game_rv and ekf both exceed the official "
            "end-alignment tolerance)"
        )
    attrs = {
        "subject_id": subject,
        ...
        "source_files": ";".join(
            pu.relative_source(folder / fn, root) for fn in ("data.hdf5", "info.json")
        ),
