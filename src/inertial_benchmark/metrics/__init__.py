...
# 选模时的跨序列聚合统计量：由**独立配置键** ``fitness_stat`` 选择，而不是给指标名加后缀。
# ``fitness`` 只写指标名，因此 ``fitness=dir_err_median fitness_stat=median`` 含义清晰
# （序列内窗口角度中位数的跨序列中位数），不存在字符串重载带来的歧义。
# 中位数适用于 val 划分小、构成偏斜的数据集：RIDI 自动生成的 val 只有 1 名受试者，
# 其中 2 条携带方式在 train 中不存在，均值会被这两条主导，选模信号很噪。
FITNESS_STATS = ("mean", "median")
...
def fitness_keys(t_rte: Iterable[float] = (1.0, 10.0),
    ...
    聚合统计量是另一个键 ``fitness_stat``（见 :data:`FITNESS_STATS`），不进这个列表。
    """
