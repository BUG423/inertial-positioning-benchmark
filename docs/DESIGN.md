...
- `loss(out, batch, epoch)` 的 `batch` 在训练与验证中**键完全相同**：至少含 `target`（视图坐标系
  目标，形状为 `input_spec.output_shape`）与 `imu`（模型输入，与 `forward` 收到的张量逐元素相同），
  另有 `mask`（逐输出的目标有效掩码）与可选的 `extra`。用到输入或掩码的损失因此在训练与验证两条
  路径上行为一致（`nn.base.LOSS_BATCH_KEYS` 是这一契约的断言点，Predictor 的验证路径与 Trainer
  传同一组键）。缺省实现把它们转发给 `fn(out, target, epoch, mask)`：所有损失（含模型包用
  `@register_loss("name")` 注册的专用损失，由模型 YAML 的 `loss`/`loss_kwargs` 引用）都按这个
  四参数签名实现并处理 `mask`。
...
- **`unified` 统一什么、不统一什么**（公平性口径，必须随结果一起报告）：
  - **统一**：训练预算（batch、优化器与学习率调度、权重衰减、梯度裁剪，以及**看过的训练窗口
    总数**）、数据与划分、窗口与步长、评测协议、随机种子集合、选模依据。
  - **训练预算按窗口数而不是 epoch 数统一**（`train_windows_budget`，见 `docs/CLI.md` §4.4）：
    各数据集每轮的训练窗口数差一个数量级（实测 RIDI 约 9.6 万、RNIN 约 47 万、PedLocData 百万级），
    固定 epoch 数等于把算力按数据集大小分配。框架按
    `epochs = clip(ceil(预算 / 每轮窗口数), 1, budget_max_epochs)` 换算出每个数据集的轮数，
    写入 `args.yaml` 的 `epochs` 与 `metrics.json` 的 `train_budget` 块。这是**配置层**能力：
    模型 YAML 不得为此硬编码 `epochs`（`official` 配方里的 `epochs` 是论文超参数，不受影响）。
    序列级模型只标定一次，不适用该预算。
  - **不统一**：算法固有的数据增强与输入表示。理由：增强对某些方法不是超参数而是方法本身——
