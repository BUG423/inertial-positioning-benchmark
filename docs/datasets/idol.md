...
| 可选依赖 | 读取 `.feather` 需要 `pyarrow`（本机验证版本 24.0.0）：`pip install -e ".[idol]"`（`pyproject.toml` 的 `idol` 可选依赖组） |
...
1. 真值世界系倾斜（已调平）以及调平后仍然存在的时变残余倾斜；`building2_known_5/6`、`building3_known_10/15/21`
   的开头 3–6 s 世界系尚未收敛（静止段倾斜 5.3–7.5°，全段 0.02°，见第 10 节）。
...
6. `building3_known_14` 的有效数据在开头 16 s 之后才开始（0.12 s 数据 + 15.85 s 缺口），转换时按 DESIGN 2.3
   裁掉了这 16.065 s。
...
输出：`/workspace/webCodex/datasets/ipb/idol`｜转换器 `idol@1.0.0`｜转换时间 2026-09-17｜
指纹 `dadeed68bbc3f119…`

接收 130 / 拒收 0，共 19.89 h、64.94 km、15 名受试者。官方无 val，流水线从 `train` 按受试者抽出 5 条
（`subject03`、`subject05`，0.77 h，占 train 时长 13.3%）。

| 划分 | 序列 | 时长 (h) | 距离 (km) | 受试者 |
|---|---:|---:|---:|---:|
| `train` | 38 | 5.03 | 16.51 | 6 |
| `val` | 5 | 0.77 | 2.55 | 2 |
| `test_known` | 51 | 8.16 | 26.43 | 8 |
| `test_unknown` | 36 | 5.93 | 19.45 | 7 |
| `test`（= known ∪ unknown） | 87 | 14.09 | 45.88 | 15 |
| `test_known_building1` | 15 | 2.36 | 7.03 | 7 |
| `test_unknown_building1` | 14 | 2.15 | 5.88 | 3 |

`ipb check data=idol full=true hash=true`：**通过**（0 错误，33 警告）：6 条受试者重叠警告
（`test_known`/`train`、`test_known`/`val` 等，known 的定义即“见过的受试者”），22 条“重力检查勉强通过”
（`|g|` 偏差约 +0.23 m/s²，来自 iPhone 加速度计约 +1.9% 的比例误差），5 条静止段/全段不一致警告（见下）。
无重复内容、无未分配序列。

### 首尾裁剪与静止段的世界系

- `building3_known_14` 的原始数据在 0.12 s 处紧跟一个 15.85 s 的缺口，开头那 0.12 s 是孤立碎片：
  按 DESIGN 2.3 第 6 条裁掉开头 16.065 s，其余原样保留（本数据集唯一被裁剪的序列）。
- `building2_known_5/6`、`building3_known_10/15/21` 的静止样本全部落在开头 3–6 s，而那几秒 Stencil SLAM
  的世界系尚未收敛：静止段的重力均值倾斜 5.3–7.5°，全段均值只有 0.02°，Stencil 自带 IMU 给出同样的偏差
  （6.6° / 7.99°），60 s 分块的倾角为 0.1–3.4°。因此这是**开头数秒的参考姿态问题**，不是约定错误；
  校验按“静止段与全段任一通过即通过”的规则接收，并记警告（实现见 DESIGN 2.2）。使用这些序列时应注意
  开头几秒的姿态。
...
|---|---:|---|---:|---:|---|---:|---:|
| `building1_train_26` | 564.82 | 536.29 → 536.29 | +0.000% | 0.949 | [−0.001, −0.003, 10.027] | 0.0001 | 0.00 |
| `building1_train_5` | 477.27 | 425.69 → 425.69 | −0.000% | 0.892 | [0.007, 0.001, 10.059] | 0.0000 | 0.00 |
| `building1_unknown_2` | 522.53 | 418.80 → 418.80 | −0.000% | 0.801 | [0.001, −0.000, 10.042] | 0.0001 | 0.00 |
| `building3_known_9` | 615.46 | 474.07 → 474.06 | −0.000% | 0.770 | [−0.001, 0.004, 10.028] | 0.0001 | 0.00 |
| `building3_unknown_3` | 663.82 | 632.98 → 632.97 | −0.001% | 0.954 | [0.001, 0.001, 10.049] | 0.0002 | 0.00 |

100 Hz → 200 Hz 的多相上采样没有改变轨迹尺度（差异 < 0.001%），时长与样本数完全一致；
加速度均值的 z 分量约 10.03–10.06 m/s² 反映 iPhone 加速度计的比例误差（第 6 节），不是转换误差。
