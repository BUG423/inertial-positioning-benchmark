...
输出：`/workspace/webCodex/datasets/ipb/tlio`｜转换器 `tlio@1.0.0`｜转换时间 2026-09-17｜
指纹 `c2d0ddbe9d6ea11f…`（`dataset.json` 的 `fingerprint`）

接收 354 / 拒收 0，共 31.60 h、37.63 km、9 个设备组；全部序列 `valid` 比例 100%（原始数据严格 200 Hz、无缺口，
IMU 与位姿共用同一时钟，重采样只做恒等线性插值）。

| 划分 | 序列 | 时长 (h) | 距离 (km) | 设备组 |
|---|---:|---:|---:|---:|
| `train` | 283 | 25.30 | 29.63 | 9 |
| `val` | 35 | 3.13 | 4.14 | 6 |
| `test` | 36 | 3.18 | 3.85 | 5 |

`ipb check data=tlio full=true hash=true`：**通过**（0 错误，3 警告）。三条警告都是官方随机划分造成的
`group_id`（设备）重叠：`test/train` 5 组、`test/val` 5 组、`train/val` 6 组，属数据集本身的性质（第 5 节）。
无重复内容、无未分配序列。
...
|---|---:|---|---:|---:|---|---:|---:|
| `145820422949970` | 257.20 | 130.57 → 130.57 | +0.000% | 0.508 | [−0.002, 0.004, 9.815] | 0.0000 | 0.00 |
| `203487666948423` | 272.10 | 131.25 → 131.25 | +0.000% | 0.482 | [0.003, 0.003, 9.809] | 0.0000 | 0.00 |
| `220702031859866` | 329.74 | 396.77 → 396.77 | +0.000% | 1.203 | [−0.001, −0.001, 9.811] | 0.0000 | 0.00 |
| `368823013640377` | 290.76 | 192.72 → 192.72 | +0.000% | 0.663 | [−0.004, −0.002, 9.810] | 0.0000 | 0.00 |
| `396634770816386` | 232.51 | 75.95 → 75.95 | +0.000% | 0.327 | [0.009, 0.000, 9.809] | 0.0000 | 0.00 |

时长与样本数完全一致（例如 257.205 s → 51442 = ⌊257.205×200⌋+1）；稠密（逐样本）路径长度差异也 < 0.005%。
