...
def write_ridi_csv(path, motion, ori, rv, time_ns, acce=None, linacce=None, grav=None):
    ...
    grav = pu.quat_rotate(pu.quat_conj(motion["q_wb"]), [0.0, 0.0, pu.GRAVITY]) if grav is None else grav
    linacce = motion["specific_force"] - grav if linacce is None else linacce
    body = np.column_stack(
        [time_ns, motion["gyro"], acce, linacce, grav, np.zeros((n, 3)), motion["position"], ori, rv]
    )
...
def test_quaternion_helpers_match_scipy():
    ...
    np.testing.assert_allclose(pu.quat_rotate(pu.Q_ZUP_FROM_YUP, [1.0, 2.0, 3.0]), [1.0, -3.0, 2.0], atol=1e-12)
    cont = pu.quat_make_continuous(np.repeat(q[:1], 6, axis=0) * np.array([1, -1, 1, -1, 1, -1])[:, None])
...
def test_monotonic_mask_and_csv_reader(tmp_path):
    np.testing.assert_array_equal(pu.monotonic_mask([0, 1, 1, 3, 2, 4]), [True, True, False, True, False, True])
    path = tmp_path / "x.csv"
    path.write_text(",a,b\n0,1.5,2\n1,3,4e3\n")
...
def test_physics_check_accepts_truth_and_rejects_wrong_conventions(motion):
    t = motion["t"]
    good = RawSequence("s", t, motion["gyro"], motion["specific_force"], t, motion["position"], motion["q_wb"])
    ...
    slow = RawSequence("s", t, motion["gyro"], motion["specific_force"], t, motion["position"] * 1e-3, motion["q_wb"])
    ...
    stats = pu.physics_check(RawSequence("s", t, motion["gyro"], motion["specific_force"], t, motion["position"], yawed))
...
def test_ridi_official_splits_and_duplicates(tmp_path, motion):
    ...
    time_ns[5] = time_ns[4]
    write_ridi_csv(root / "tang_body9" / "processed" / "data.csv", motion, motion["q_wb"], motion["q_wb"], time_ns)
